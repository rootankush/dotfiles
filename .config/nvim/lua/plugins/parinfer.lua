...
local function shiftCursorOnEdit(result, lineNo, startIdx, endIdx, replaceTxt)
  ...
  if
    dx ~= 0
    and result.cursorLine == lineNo
    and result.cursorX ~= UINT_NULL
    and isCursorAffected(result, startIdx, endIdx)
  then
...
local function checkCursorHolding(result)
  ...
  if shouldCheckPrev then
    local prevHolding = result.prevCursorLine == opener.lineNo
      and holdMinX <= result.prevCursorX
      and result.prevCursorX <= holdMaxX
...
local function isCursorClampingParenTrail(result, cursorX, cursorLine)
  return (
    ...
    and not isCursorInComment(result, cursorX, cursorLine)
  )
end
